...
-- Manus GUI Library V7.0 (Custom Additions by Gemini)
local Library = {}
...
function Library:TeleportToPlayer(targetPlayer)
    if tick() < Library.TeleportCooldownUntil then return end
...
--[[
    4. JANELAS PRONTAS (Whitelist, etc)
]]
-- O código das janelas prontas como Whitelist e Killaura permanece o mesmo...

--[[
    5. API DE JANELAS E CATEGORIAS (COM NOVAS FUNÇÕES)
]]
function Library:CreateWindow(title, size, position)
    ...
    -- O UIListLayout padrão é movido para ser opcional.
    -- local l=Instance.new("UIListLayout",ContentFrame); l.Padding=UDim.new(0,8); l.HorizontalAlignment=Enum.HorizontalAlignment.Center; Instance.new("UIPadding",ContentFrame).PaddingTop=UDim.new(0,8)
    ...
    function windowObj:AddButton(t,c) 
        local b=Instance.new("TextButton"); b.Size=UDim2.new(0.9,0,0,32); b.BackgroundColor3=Color3.fromRGB(50,50,50); b.TextColor3=Color3.fromRGB(220,220,220); b.Text=t; b.TextSize=14; b.Font=Enum.Font.SourceSansBold; b.Parent=ContentFrame; Instance.new("UICorner",b); b.MouseButton1Click:Connect(c); 
        
        -- Adiciona um UIListLayout se ainda não houver um
        if not ContentFrame:FindFirstChildOfClass("UIListLayout") then
            local l=Instance.new("UIListLayout",ContentFrame); l.Padding=UDim.new(0,8); l.HorizontalAlignment=Enum.HorizontalAlignment.Center;
        end
    ...
    function windowObj:AddTextBox(p) 
    ...
    -- ===============================================================
    -- ====== NOVA FUNÇÃO: AddLabel (Corrigido por Gemini) ===========
    -- ===============================================================
    ...
    -- =======================================================================
    -- ====== NOVA FUNÇÃO: AddScrollableList (Criado por Gemini) =============
    -- =======================================================================
    function windowObj:AddScrollableList(position, size)
        local frame = Instance.new("ScrollingFrame")
        frame.Size = size or UDim2.new(1, -10, 1, -50)
        frame.Position = position or UDim2.new(0, 5, 0, 5)
        ...
        frame:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
            listLayout.Parent = nil -- Workaround for layout bug
            listLayout.Parent = frame
        end)
        
        -- Retorna o frame da lista para que botões e textos possam ser adicionados a ele
        return frame
    end
...
-- As funções de Categoria, Módulo, etc, permanecem as mesmas
...
UserInputService.InputBegan:Connect(function(i,p) if not p and i.KeyCode==Library.OpenKey then MainFrame.Visible=not MainFrame.Visible end end)
